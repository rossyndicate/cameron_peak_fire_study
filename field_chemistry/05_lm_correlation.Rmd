...
t_test_df <- all_param_df%>%
  mutate(t_test_model = map(data, ~ t.test(.x$value)),
         # p_value = map(data, ~ t.test(.x$value)$p.value),
         # t_statistic = map(data, ~ t.test(.x$value)$statistic),
         # df = map(data, ~ t.test(.x$value)$parameter),
  ...
  mutate(t_test = as.numeric(t_test),
         ...
         sd = as.numeric(sd),
         n = as.numeric(n))%>%
  select(site_code, param, t_test, p_value, t_statistic, df, mean, sd, n)
