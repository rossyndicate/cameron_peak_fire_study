...
comparison_plots_df <- filter(reservoir_chemistry, site_code %in% c("CBRR","CHD","JOEI", "JOER", "CBRI", "BRNR" ,"BMD")& Year >2020)%>%
  bind_rows(cbri_data)%>%
...
comparison_plot <-  ggplot(compare_df)+
  geom_point(aes(x=Date, y = value, color=watershed, shape=watershed), size= 2)+
  geom_path(aes(x=Date, y = value, group=site_code, color = watershed))+
...
NO3<- plot_comparison_timeline("NO3")
ChlA <- plot_comparison_timeline("ChlA")
DOC <- plot_comparison_timeline("DOC")
DTN <- plot_comparison_timeline("DTN")
K <- plot_comparison_timeline("K")
