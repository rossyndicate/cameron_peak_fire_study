...
reservoir_chemistry <- read_csv('data/ReservoirChemistry_010523.csv')%>%
...
summary(reservoir_chemistry)

# number of samples per site per year
res_samples_year_site <- reservoir_chemistry%>%
  group_by(Year, site_code)%>%
  dplyr::summarise( n = n())

summary(res_samples_year_site)


#Determning number of Samples....

rmrs_chemistry <- read.csv("data/CamPkChem.csv")%>%
...
res_icp <- read_csv("data/ReservoirStudy_ICPData.csv")%>%
  mutate(Date = as.Date(Date, format = "%d-%b-%y"), 
...
res_all <- reservoir_chemistry%>%
  left_join(res_icp, by = "FCW_Number")%>%
    # select(-c(IDNo, 
    #         SiteDescr,
    #         watershed, 
    #         Lat, Long,
    #         order, Xdistance, SampleType, Location, Distance, Site, SiteLabel))%>%
...
write_feather(res_all, sink = "data/res_chem_cleaned/res_all_chem")

  
# ggplot(data = filter(res_all, Year >=2021& Parameter == ""), aes(x = Date, y = Measurement, group = site_code, color = site_code))+
#   geom_point()+
...
#Fcgov sites in our study
fcgov_study <- c("CHD", "BMD","JWC", "PJW", "PBR", "SFM", "PSF", "PNF", "PBD")

#importing 
fcgov_mainstem <- read.csv("data/fcgov_mainstem_chem.csv", na.strings = '*')%>%
 ...
 dplyr::rename(NO3 = NO3_N, 
  ...
  dplyr::select(site_code,Year, Date ,Turbidity, pH, SC, Na, K, Mg, Ca, Cl, NO3, PO4, SO4, DO, Temp,collector, status )

write_feather(fcgov_mainstem, sink = "data/res_chem_cleaned/fcgov_mainstem")
