...
chla_long <- main_res_chla_longitudinal%>%
  ggplot() +
   ...
   geom_point(aes(x=Distance, y=ChlA, shape = location_type, color=Date), size=8, alpha = .7) +
   scale_color_manual(values=c('#FA850E','#397534','#59B851','#DEB907','#047E82'))+
   scale_fill_manual(values=c('#FA850E','#397534','#59B851','#DEB907','#047E82'))+
...
ggarrange(chla_long,no3_long, ncol=1, nrow=2, common.legend=T)
 ...
 ggsave('output/2022_joer_cbrr_chla_no3_longitudinal.jpg', width=13.5, height=10)
 ...
 ggsave('output/2022_joer_cbrr_chla_no3_longitudinal_highlighted.jpg', width=13.5, height=10)
```
...
ggsave('output/main_study_reservoirs_timeline_boxplot.jpg',width=15,height=10)
...
ggplot(aes(x=reorder(site_year,order,max),y=ChlA, middle=mean(ChlA), fill=Watershed_Level, colour = factor(Year))) + 
  ...
  geom_abline(aes(slope=0, intercept=8), color='black', linetype="dashed") +
  #annotate(geom="text", label= "MDL Limit (μg/L)", x="CBRR 2022", y=.075, vjust= -1)+
  annotate(geom="text", label= "Mesotrophic", x=1.5, y=2.6, vjust= -1, size = 5)+
...
```{r}
 xaxis_dates <-c("June 1st", "July 1st",  "August 1st", "Sept 1st",  "October 1st" )
#CHLA timeline
...
chla_timeline <- study_reservoir_chla_res%>%
  ...
  theme(legend.position = "none")+
  coord_cartesian(ylim=c(0,12))+
  scale_x_continuous(breaks = c(150,181,215,245,275),labels=xaxis_dates)+
    gghighlight(site_year == "CBRR 2022"|site_year == "JOER 2022"|site_year == "BRNR 2022", keep_scales = T, use_direct_label = T)

 

plot(chla_timeline)

ggsave("output/chla_timeline_JOER_CBRR_BRNR_2022.jpg", width = 11, height = 9)
...
longitudinal_res_mainstem <- filter(reservoir_chemistry, Year == "2022")%>%
...
longitudinal_res_mainstem$wk <- factor(longitudinal_res_mainstem$wk, levels = c("22","28","30","32", "36", "38", "40", "42"))
...
scale_color_manual(name = "Week of", labels= c("6-01-22",
                                               ...
                                               "8-08-22", 
                                               "9-05-22",
                                               "9-19- 22"), values = c("22" = "#047E82", "28" = "#397534", "30"='#59B851', "32"= '#DEB907', "36" = '#CC79A7', "38" = '0072B2' ))+
  scale_shape_manual(values=c(17, 16, 15))+
  scale_fill_manual( values = c("22" = "#047E82", "28" = "#397534", "30"='#59B851', "32"= '#DEB907', "36" = '#CC79A7', "38" = '0072B2' ))+
...
ggplotly(longitudinal_graph_res_mainstem)
...
#gghighlight graphs for presentation
  ...
  longitudinal_gghighlight_graph_res_mainstem <- longitudinal_graph_res_mainstem+
    gghighlight(wk == "32", keep_scales = T, use_direct_label = FALSE)
...
res_ratio_plot <- filter(joer_cbrr_ratios, Date != "2022-06-14")%>%
 ...
 # theme(axis.title.y = element_text(angle = 0, vjust = .5)) +
  ...
  theme(legend.justification = c(.05, .9), legend.position = c(.05, .9),legend.box = "horizontal",
          axis.title = element_text(face = "bold"),
         axis.text.x = element_text(face = "bold"))
ggsave("output/Res_Ratio.jpg",width = 24, height = 12)
#plot(res_ratio_plot)
...
barnes_effect_timeline <- barnes_effect_dataset%>%
  ggplot()+
  geom_path(aes(x= wk, y= ChlA, group = site_code, color = site_code), size = 2)+
  geom_point(aes(x= wk, y= ChlA, color = site_code), size= 5)+
  theme_bw(base_size = 24)

plot(barnes_effect_timeline)
```
