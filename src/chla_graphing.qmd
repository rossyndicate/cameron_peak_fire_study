...
geom_boxplot(aes(x=reorder(site_code,order,max),y=ChlA, middle=mean(ChlA), fill=Watershed_Level))+
   ...
   geom_dotplot(aes(x=reorder(site_code,order,max),y=ChlA, middle=mean(ChlA), fill = Watershed_Level), stackgroups = FALSE, binaxis = "y", method = "histodot",stackdir = "center", position = "dodge", binwidth = .5, dotsize = 1, alpha = .7)+
...
ggsave('output/main_study_reservoirs_timeline_dot_and_boxplot.jpg',width=15,height=10)
...
barnes_effect_dataset$site_code <- factor(barnes_effect_dataset$site_code, levels = c("CBRR", "BRNR","CHD", "BMD", "JWC","SLEP","PBR"))
