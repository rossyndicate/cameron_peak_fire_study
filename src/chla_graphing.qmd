...
#NO3
 ...
 ggsave('output/CHLA_LONG_SF.jpg', width=15, height=15)
...
mainstem_chla_long <- ggplot() +
   geom_point(data=Mainstem_chla_day , aes(x=Distance, y=ChlA, shape = Location, color=factor(Date)), size=5) +
   geom_line(data=Mainstem_chla_day, aes(x=Distance, y=ChlA, group = Date ,color=factor(Date)), size=1) +
   geom_point(data=Mainstem_chla_day, aes(x=Distance, y=ChlA, shape = Location, color=factor(Date)), size=5) +
   #scale_color_manual(values=c('#047E82','#397534','#59B851','#DEB907','#FA850E')) +
   #scale_fill_manual(values=c('#047E82','#397534','#59B851','#DEB907','#FA850E')) +
   theme_bw(base_size=24) +
...
Mainstem_no3_long <- ggplot() +
   geom_point(data= Mainstem_chla_day, aes(x=Distance, y=NO3, shape = Location, color=factor(Date)), size=5) +
   geom_line(data=Mainstem_chla_day, aes(x=Distance, y=NO3, group = Date, color=factor(Date)), size=1) +
   geom_point(data=Mainstem_chla_day, aes(x=Distance, y=NO3, shape = Location, color=factor(Date)), size=5) +
   #scale_color_manual(values=c('#047E82','#397534','#59B851','#DEB907','#FA850E'))+
   #scale_fill_manual(values=c('#047E82','#397534','#59B851','#DEB907','#FA850E')) +
   theme_bw(base_size=24) +
...
Key_Mainstem_Chemistry <- filter(Mainstem_Chemistry, Site_Code %in% c("JWC","PBR","PSF","PNF","PBD"))
...
Mainstem_chla_timeline <- Key_Mainstem_Chemistry%>%
  ...
  geom_point(data = Key_Mainstem_Chemistry, aes(x=Date,y=ChlA, color=factor(order)), size=3.5)+
  
  geom_textpath(data = dplyr::filter(Mainstem_Chemistry, Site_Code %in% c("JWC")), aes(x=Date, y=ChlA, group=Site_Code, color=factor(order), size=3, label=Site_Code), size=4.5, fontface=2, hjust=0.1, vjust=0.0) +
  
  geom_textpath(data = dplyr::filter(Mainstem_Chemistry, Site_Code %in% c("PBR", "PSF")), aes(x=Date,y=ChlA, group=Site_Code, color=factor(order), size=3, label=Site_Code), size=4.5, fontface=2, hjust=0.5, vjust=0) +
  
  geom_textpath(data = dplyr::filter(Mainstem_Chemistry, Site_Code %in% c("PBD", "PNF")), aes(x=Date,y=ChlA, group=Site_Code, color=factor(order), size=3, label=Site_Code), size=4.5, fontface=2, hjust=0.9, vjust=0)+
...
###CHLA  BOXPLOT GRAPH ###
Mainstem_chla_boxplot <- filter(Mainstem_Chemistry, Site_Code %in% c("JWC","SLEP","PBR","PSF","PNF","PBD"))%>%
ggplot() + 
