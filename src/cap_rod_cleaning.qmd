...
#IF stage is the same, don't measure discharge


##ENTER THE FUNCTION ZONE

manual_h_q <- read.csv("data/manual_values_dec_2022.csv")%>%
  na.omit(Q_cfs)%>%
  filter(site_code != "LBEA" & site_code != "SHEP")%>%
  select(H = first_measurement_cm, Q = Q_cfs, site_code)





df_for_q_test <- final_df%>%
  filter(site_code != "LBEA1" &site_code != "LBEA2"&site_code != "SHEP1" &site_code != "SHEP2")

unique(df_for_q_test$site_code)


create_rating_curve <- function(site){
  
  site_manual_h_q <- manual_h_q%>%
    dplyr::filter(site_code == site)%>%
    mutate(logH = log10(H),
           logQ = log10(Q))
...
site_list <- data.frame(site = c("FISH", "SAWM", "BLAK", "PENN", "BEAV", "BENN"))%>%
  as_tibble()

#using manual dataframe to create coefficients for each site
manual_H_Q_check <- pmap_dfr(site_list, create_rating_curve)

#double check that modelled Q matches measured Q
double_check_model <- manual_H_Q_check%>%
  ggplot()+
...
calc_Q_for_sensor_stage_15min <- function(site){
  ...
  site_rc <- manual_H_Q_check%>%
    dplyr::filter(site_code == site)
  
  #filter corrected stage df for site
   stage_Q_df <- df_for_q_test%>%
     dplyr::filter(site_code == site)%>%
...
stage_Q_test <- pmap_dfr(site_list, calc_Q_for_sensor_stage)




double_check_stage_Q <- filter(stage_Q_test, site_code == "SAWM")%>%
  ggplot()+
