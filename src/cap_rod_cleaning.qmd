...
##Function to give dataset a time correction factor for each subset
    ...
    blak_factor <- filter(factor_df, site_code == "BLAK")
...
```{r}
 ...
 indv_corrected_df <- filter(corrected_df, site_code == "PENN")
 indv_manual_measurements <- filter(manual_measurements_for_func, site_name == "PENN")
...
all_corrected_graph <- ggplot()+
  geom_line(data = corrected_df, aes( x= DT , y= water_height_cm), color= "red")+
  geom_line(data = corrected_df, aes( x= DT , y= corrected_stage_cm), color= "black")+
  geom_line(data = corrected_df, aes( x= DT , y= adjusted_stage), color= "green", alpha = .5)+
  geom_point(data =  manual_measurements_for_func, aes( x= start_date , y= manual_start), color = "blue")+
   theme_bw()+
   facet_wrap(~site_code)

ggplotly(indv_corrected_graph)

corrected_only_graph <- ggplot()+
  geom_line(data = indv_corrected_df, aes( x= DT , y= water_height_cm), color= "red")+
  geom_line(data = indv_corrected_df, aes( x= DT , y= corrected_stage_cm), color= "black")+
  geom_point(data =  indv_manual_measurements, aes( x= start_date , y= manual_start), color = "blue")+
   theme_bw()

ggplotly(corrected_only_graph)
