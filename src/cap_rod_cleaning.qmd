...
stage_df <- read_and_combine(folder_path = "~/Repositories/cameron_peak_fire_study/data/all_sites_caprod_data",
  ...
  ##can add other columns to select if you want
  ...
  mutate(water_height_cm = wtrhgt__5 /10, 
        ...
        #in case function doesn't remove excess chars for site_code
        site_code = str_extract(string = site_code, 
                                      pattern = "[A-Z]+"))%>%
  #not required to save mm but can be nice to make sure everything is working correctly
  dplyr ::rename(water_height_mm = wtrhgt__5)



#Pull in field measured values
#change csv to whatever your manual values are store in 
##SEE RULES FOR COLUMNS NAMES
manual_measurements <- read.csv("data/manual_values_dec_2022.csv")%>%
 
      mutate(first_measurement_DT = as.POSIXct(first_measurement_DT, 
      ##IF DTs are not reading correctly, you may need to adjust formats
...
-   check for missing data, weird dates, see if there are any 0s near sites
...
## Export uncleaned combined stage

```{r}
write_csv(stage_df, "data/Provisional_Q_stage_data/stage_combined_UNCLEAN.csv")
```
...
##Function to adjust sensor stage values to match start measurement  
  ...
  adjust_stage <- function(site_name,start_date, end_date, manual_start, manual_end){
    stage_df_new<- stage_df%>%
      #subset df by site and datetimes between site visits
     dplyr::filter(stage_df$site_code %in% site_name)%>%
     dplyr::filter(between(DT, start_date, end_date))%>%
      #difference between manual measurement and the average of the first 4 measurements in the subset 
    #ie value that sensor sees near first site visit
...
##Function to give dataset a time correction factor for each subset
    ...
    factor_df <-stepped_df%>%
     ...
     dplyr::filter(between(DT, start_date, end_date))%>%
      #dr factor needs date/time to create time factor
      mutate(time =  format(DT, format = "%H:%M"), 
             date = as.Date(DT, format = "%Y-%m-%d"))%>%
      #arranging subset seems to correct factor errors
      arrange(DT)%>%
      #creating time factor for subset of data
...
#Function changes site_code to new names in rename_df if between(start/end date)
...
#all of the sites with no changes
...
-   TREND: Magnitude of data is correct but data trends in an unintuitive way

-   VARIATION: Daily variation is questionable

    -   use larger timestep for safety

-   Any additional: explain in notes
...
write.csv(final_stage_df, "data/Provisional_Q_stage_data/corrected_15min_stage_CPF_2022.csv")
...
#join samples by date and stage DFs
...
#plot to make sure it looks correct/ no data is missing. 
...
write_csv(final_daily_means_stage, "data/Provisional_Q_stage_data/stage_daily_mean_and_samples_cleaned.csv")
...
This section contains 2 ways to calculate rating curves: 1. For datasets where there are few measurements, the function Q \~ (a\*H)\^b

2.  For datasets with more Q/stage measurements, the package "bdrc" may be a more accurate solution This package calculates more error guidelines but takes a while to run.

### Playing with rating curve package 'bdrc'

FOR LARGER DATASETS USE THIS PACKAGE. NEEDS FUNCTION DEVELOPMENT TO MAP THROUGH MULTIPLE SITES CODE IN CHUNK IS EXAMPLE DATASET PROVIDED WITH PACKAGE
...
-   calc_Q\_for_sensor_stage_15min

-   calc_Q\_for_sensor_stage_daily

-   EXTRAPOLATION_Q\_sensor_15min

-   EXTRAPOLATION_Q\_sensor_daily
...
create_rating_curve <- function(site){
  ...
  #pulling coefficients in to manual DF
...
#USE EXTRAPOLATED AT OWN RISK, ERROR MAY BE PROPEGATED

EXTRAPOLATED_stage_Q_15min <- pmap_dfr(site_list, EXTRAPOLATION_Q_sensor_15min)

extrapolated_stage_Q <-
  ggplot()+
  geom_line(data= filter(stage_Q_15min , site_code == "SAWM"),aes(x= DT, y = Q_cfs, color = "blue"))+
  geom_line(data= filter(EXTRAPOLATED_stage_Q_15min, site_code == "SAWM"),aes(x=DT, y = Q_cfs, color = "red"))+
  #scale_color_manual(values = flag_colors)+
  theme_bw()
...
extrapolated_stage_Q <-
  ggplot()+
  geom_line(data= filter(stage_Q_daily, site_code == "SHEP1"),aes(x= date, y = Q_cfs, color = "blue"))+
  geom_line(data= filter(EXTRAPOLATED_stage_Q_daily, site_code == "SHEP1"),aes(x= date, y = Q_cfs, color = "red"))+
  #scale_color_manual(values = flag_colors)+
  theme_bw()
...
Save into folder "Provisional_Q\_stage_data"

```{r}

write_csv(stage_Q_15min, "data/Provisional_Q_stage_data/stage_Q_15min_CPF2022.csv")

write_csv(stage_Q_daily, "data/Provisional_Q_stage_data/stage_Q_15min_CPF2022.csv")

write_csv(EXTRAPOLATED_stage_Q_15min , "data/Provisional_Q_stage_data/EXTRAPOLATED_stage_Q_15min_CPF2022.csv")

write_csv(EXTRAPOLATED_stage_Q_daily , "data/Provisional_Q_stage_data/EXTRAPOLATED_stage_Q_daily_CPF2022.csv")
