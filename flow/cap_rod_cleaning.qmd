...
folder_path <- paste0(getwd(),"/data/all_sites_caprod_data")
...
field_stage <- "data/manual_values_dec_2022.csv"
...
aspen <- read_csv("data/flow_kampf/aspen_stage_composite.csv")%>%
  mutate(DT = datetime, 
         water_height_cm = Stage_mm /10, 
         site_code = "aspen")%>%
  dplyr::select(site_code, DT, water_height_cm)

greyrock <- read_csv("data/flow_kampf/greyrock_stage_composite.csv")%>%
  mutate(DT = datetime, 
         water_height_cm = Stage_mm /10, 
         site_code = "greyrock")%>%
  dplyr::select(site_code, DT, water_height_cm)

bl4 <- read_csv("data/flow_kampf/bl4_stage_composite.csv")%>%
  mutate(DT = datetime, 
         water_height_cm = Stage_mm /10, 
         site_code = "bl4")%>%
  dplyr::select(site_code, DT, water_height_cm)
...
kampf_stage_df <- rbind(aspen, greyrock, bl4)%>%
...
```{r}
master_visual <- kampf_stage_df %>%
  ggplot(aes(x = DT, y = water_height_cm, color = site_code)) +
  geom_line()
plot(master_visual)

indv_visual <- filter(kampf_stage_df, site_code == "bl4") %>%
  ggplot(aes(x = DT, y = water_height_cm, color = site_code)) +
  geom_line()
plot(indv_visual)
```
...
sample_plot <- filter(daily_means_stage_w_samples, site_code == "FISH") %>%
  ...
  geom_line(aes(x = date, y = mean_sensor_stage_cm, group = site_code, 
                color = flag_type)) +
  scale_color_manual(values = flag_colors) +
  # geom_line(aes(x = date, y = mean_corrected_stage_cm, 
  # color = "red" ))+
...
write_csv(final_daily_means_stage, 
          "data/Provisional_Q_stage_data/stage_daily_mean_and_samples_cleaned.csv")
```
...
extrapolated_stage_Q <-
  ggplot() +
  geom_line(data = filter(stage_Q_daily, site_code == "SHEP1"),
            aes(x = date, y = Q_cfs, color = "blue")) +
  geom_line(data = filter(EXTRAPOLATED_stage_Q_daily, site_code == "SHEP1"),
