...
sites <- most_recent_meta%>%
  ...
  mutate( map_2021_2022  = case_when(
   ...
   map_2023  = case_when(
    site_code %in% c( "JOER",  "CBRR", "BRNR", "PTRR", "LNGR", "JWC", "PJW", "PBR", "PSF", "PNF", "COMR", "HORR", "BEAV") ~ "bi and monthly", 
...
map <- mapview::mapview(filter(sites, map_2023 == "trib"), layer.name = "trib",col.regions = "#999999", cex = 5, alpha =1)+
  mapview::mapview(filter(sites, map_2023 == "bi and monthly" ), layer.name = "biweek",col.regions = "#03ffff", cex = 9,alpha = 1)+ mapview::mapview(filter(sites, map_2023 == "sensor"), layer.name =  "weekly",col.regions = "#ff0303", cex = 10, alpha = 1 )
